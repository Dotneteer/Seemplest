...
namespace Seemplest.MsSql.UnitTests.DataAccess
{
    ...
    public class FetchAsyncTest
    {
        ...
        public async Task FetchAsyncWorksWithMissingClrField()
        {
            ...
            // --- Act
            var rows = await db.FetchAsync<SampleRecordWithMissingField>("select Id, Name from sample order by Name");
        ...
        public async Task FetchAsyncWorksWithMissingDataField()
        {
            ...
            // --- Act
            var rows = await db.FetchAsync<SampleRecordWithExtraField>("select Id, Name from sample order by Id");
        ...
        public async Task FetchWorksWithNarrowingConversion()
        {
            ...
            // --- Act
            var rows = await db.FetchAsync<SampleRecordWithNarrowingConversion>("select Id, Name from sample order by Id");
        ...
        public async Task FetchAsyncWorksWithWideningConversion()
        {
            ...
            // --- Act
            var rows = await db.FetchAsync<SampleRecordWithWideningConversion>("select Id, Name from sample order by Id");
        ...
        public async Task FetchAsyncWorksWithTypeConversion()
        {
            ...
            // --- Act
            var rows = await db.FetchAsync<SampleRecordWithTypeConversion>("select Id, Name from sample order by Id");
        ...
        public async Task FetchAsyncWorksWithIntegerBasedEnumValues()
        {
            ...
            // --- Assert
            rows.ShouldHaveCountOf(2);
            rows[0].Id.ShouldEqual(SqlDbType.Binary);
            rows[0].Name.ShouldEqual("First");
            rows[1].Id.ShouldEqual(SqlDbType.Bit);
            rows[1].Name.ShouldEqual("Second");
        }
        ...
        public async Task FetchAsyncWorksWithNonIntegerBasedEnumValues()
        {
            // --- Arrange
            var db = new SqlDatabase(DB_CONN);
            await db.ExecuteAsync(@"create table sample(Id smallint not null, Name varchar(50) null)");
            await db.ExecuteAsync("insert into sample values(1, 'First')");
            await db.ExecuteAsync("insert into sample values(2, 'Second')");
        ...
        public async Task FetchAsyncWorksWithEnumStringValues()
        {
            // --- Arrange
            var db = new SqlDatabase(DB_CONN);
            await db.ExecuteAsync(@"create table sample(Id varchar(20) not null, Name varchar(50) null)");
            await db.ExecuteAsync("insert into sample values('Binary', 'First')");
            await db.ExecuteAsync("insert into sample values('Bit', 'Second')");
        ...
        public async Task InvalidMappingRaisesAnException()
        {
            // --- Arrange
            var db = new SqlDatabase(DB_CONN);
            await db.ExecuteAsync(@"create table sample(Id bit not null, Name varchar(50) null)");
            await db.ExecuteAsync("insert into sample values(0, 'First')");
            await db.ExecuteAsync("insert into sample values(1, 'Second')");
        ...
        public async Task FirstAsyncIntoWorksAsExpected()
        {
            ...
            var row1 = await db.FirstIntoAsync(instance, "select Name from sample order by Id");
            var row2 = await db.FirstIntoAsync(instance, SqlExpression.CreateFrom("select Name from sample order by Id"));
        ...
        public async Task FirstOrDefaultAsyncIntoWorksAsExpected()
        {
            ...
            // --- Act
            var instance = new SampleRecord { Id = 3 };
            var row1 = await db.FirstOrDefaultIntoAsync(instance, "select Name from sample order by Id");
            var row2 = await db.FirstOrDefaultIntoAsync(instance, SqlExpression.CreateFrom("select Name from sample order by Id"));
            var row3 = await db.FirstOrDefaultIntoAsync(instance, "select Name from sample where Id = 3");
            var row4 = await db.FirstOrDefaultIntoAsync(instance, SqlExpression.CreateFrom("select Name from sample where Id = 3"));
        ...
        public async Task SingleAsyncWorksAsExpected()
        {
            ...
            var row2 = await db.SingleAsync<SampleRecord>(new SqlExpression("select Id, Name from sample where Id = 2"));
